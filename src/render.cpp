...
void renderImage(const Scene& scene, const BVHInterface& bvh, const Features& features, const Trackball& camera, Screen& screen)
{
    ...
    if (features.extra.enableDepthOfField) {
        ...
        // Changed declaration notation since omp was returning an error, where the for loop must be the first code encountered,
        const int render_step = screen.resolution().y / 10;
...
void multisamplingVisualDebug(const std::vector<Ray>& rays) {
    for (auto ray : rays) {
        drawRay(ray, glm::vec3 { 211.f / 255.f, 84 / 255.f, 0 });
        // renderRay(state, ray);
    }
}
...
std::vector<Ray> generatePixelRaysMultisampled(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    // Reference I used to understand the theory of multisampling: https://medium.com/@muhammedcan.erbudak/ray-tracing-from-scratch-multisampling-distribution-ray-tracing-6fa2904cd109
    auto numSamples = state.features.numPixelSamples;

    glm::vec2 range = 2.0f / glm::vec2(screenResolution);
    glm::vec2 pos = glm::vec2(pixel) / glm::vec2(screenResolution) * 2.f - 1.f;
    std::vector<Ray> rays(numSamples);

    for (uint32_t i = 0; i < numSamples; i++) {
        rays[i] = camera.generateRay(pos + state.sampler.next_2d() * range);
    }
    multisamplingVisualDebug(rays);
    return rays;
}
...
std::vector<Ray> generatePixelRaysStratified(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    glm::vec2 step = 2.0f / glm::vec2(screenResolution) / (float) numSamples;
    glm::vec2 pos = glm::vec2(pixel) / glm::vec2(screenResolution) * 2.f - 1.f;
    float x_0 = pos.x;
    std::vector<Ray> rays;
    ...
    for (uint32_t i = 0; i < numSamples; i++) {
        for (uint32_t j = 0; j < numSamples; j++) {
            rays.push_back(camera.generateRay(pos + state.sampler.next_2d() * step));
            pos.x += step.x;
        }
        pos.x = x_0;
        pos.y += step.y;
