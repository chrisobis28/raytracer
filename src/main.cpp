...
int main(int argc, char** argv)
{
    ...
    if (!config.cliRenderingEnabled) {
        ...
        bool debugSceneTransform { false };
#if ENABLE_MOTION_BLUR
        bool debugBVHLevelTime { false };
        int selectedMesh = 0;
#endif
        ...
        while (!window.shouldClose()) {
...
#if ENABLE_MOTION_BLUR
                    updateSceneWithTransforms(scene);
#endif
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableBvhSahBinning) {
                    ImGui::Indent();
                    ImGui::Checkbox("SAH DEBUG", &config.features.extra.sahDebug);
                    if (config.features.extra.sahDebug) {
                        ImGui::Indent();
                        ImGui::SliderInt("BVH Leaf", &nodeNumber, 0, bvh.nodes().size()-1);
                        ImGui::Unindent();
                    }
                    ImGui::Checkbox("Ray Intersect Debug", &config.features.extra.intersectDebug);
                    ImGui::Unindent();
                }
                ...
                if (config.features.extra.enableMotionBlur) {
                    ImGui::Indent();
#if ENABLE_MOTION_BLUR
                    uint32_t minSamples = 100u, maxSamples = 4000u;
                    // Number of samples in time
                    ImGui::SliderScalar("Time samples", ImGuiDataType_U32, &config.features.extra.numTimeSamples, &minSamples, &maxSamples);
                    // Shutter speed
                    ImGui::SliderFloat("Shutter speed", &config.features.extra.shutterSpeed, 0.f, 1.f);
#else
                    ImGui::Text("ENABLE_MOTION_BLUR flag in \"mesh.h\" is set to false, set it to true and recompile.");
#endif
                    ImGui::Unindent();
                }
                ...
                if (config.features.extra.enableGlossyReflection) {
                    ...
                    ImGui::Checkbox("Glossy Debug", &config.features.extra.enableGlossyDebug);
                    ImGui::Unindent();
                }
...
#if ENABLE_MOTION_BLUR
                ImGui::Checkbox("Draw scene transforms", &debugSceneTransform);
                if (debugSceneTransform && ImGui::BeginCombo("Mesh number", std::to_string(selectedMesh).c_str())) {
            ...
            case ViewMode::Rasterization: {
                ...
                if (debugBVHLevel || debugBVHLeaf || config.features.extra.sahDebug || debugSceneTransform) {
                    glPushAttrib(GL_ALL_ATTRIB_BITS);
                    setOpenGLMatrices(camera);
                    glDisable(GL_LIGHTING);
                    ...
                    if (debugBVHLevel) {
#if ENABLE_MOTION_BLUR
                        bvh.debugDrawLevelTime(bvhDebugLevel);
#else
                        bvh.debugDrawLevel(bvhDebugLevel);
#endif
                    }
                    if (config.features.extra.sahDebug)
                        bvh.sahDebug(nodeNumber);
                    if (debugBVHLeaf)
                        bvh.debugDrawLeaf(bvhDebugLeaf);
#if ENABLE_MOTION_BLUR
                    if (debugSceneTransform) {
                        // Draw transform for selected mesh or a sphere when there are no meshes
                        if (!scene.meshes.empty()) {
                            drawTransform(scene.meshes[selectedMesh].transform, scene.meshes[selectedMesh].vertices[0].position, config.features.extra.shutterSpeed);
                        } else if (!scene.spheres.empty()) {
                            drawTransform(scene.spheres[0].transform, scene.spheres[0].center, config.features.extra.shutterSpeed);
                        }
