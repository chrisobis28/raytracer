...
BVH::BVH(const Scene& scene, const Features& features)
{
    ...
    if (features.extra.enableMotionBlur) {
        ...
        buildRecursiveTime(scene, features, primitives, RootIndex, 0, glm::vec2 {0, 1});
#endif
    } else {
...
{
    //minimum and maximum coordinates of vertices
    return { .lower = glm::min(glm::min(primitive.v0.position, primitive.v1.position), primitive.v2.position), 
...
AxisAlignedBox computeSpanAABB(std::span<const BVHInterface::Primitive> primitives)
{
    ...
    for (BVHInterface::Primitive p : primitives) {
        aabbmax = glm::max(p.v2.position, glm::max(p.v1.position, glm::max(p.v0.position, aabbmax)));
        aabbmin = glm::min(p.v2.position, glm::min(p.v1.position, glm::min(p.v0.position, aabbmin)));
    }
    return { .lower = aabbmin, 
        .upper = aabbmax };
}
...
{
    //1/3 of all coordinates (intersection of medians)
    return glm::vec3 { (primitive.v0.position.x + primitive.v1.position.x + primitive.v2.position.x) / 3.0f,
        (primitive.v0.position.y + primitive.v1.position.y + primitive.v2.position.y) / 3.0f,
        (primitive.v0.position.z + primitive.v1.position.z + primitive.v2.position.z) / 3.0f };
}
...
{
    //length of each axis
    float x_axis_length = glm::abs(aabb.upper.x - aabb.lower.x);
    float y_axis_length = glm::abs(aabb.upper.y - aabb.lower.y);
    float z_axis_length = glm::abs(aabb.upper.z - aabb.lower.z);
    if (x_axis_length >= y_axis_length) {
        if (z_axis_length > x_axis_length)
            return 2;
        return 0;
    }
...
bool intersectRayWithBVH(RenderState& state, const BVHInterface& bvh, Ray& ray, HitInfo& hitInfo)
{
    ...
    if (state.features.enableAccelStructure) {
        ...
        float provisional_t = ray.t;
        BVH::Primitive debugTriangle;
        while (stack.empty() == false) {
            BVHInterface::Node curNode = stack.top();
            stack.pop();

            if (intersectRayWithShape(curNode.aabb, ray)) {
                if(state.features.extra.intersectDebug)
                    drawAABB(curNode.aabb, DrawMode::Wireframe, glm::vec3(1.0f, 0.0f, 0.5f), 1.0f);
                ray.t = provisional_t;
                if (curNode.isLeaf()) {
                    for (uint32_t i = 0; i < curNode.primitiveCount(); ++i) {
                        BVHInterface::Primitive p = primitives[curNode.primitiveOffset() + i];
                        if (intersectRayWithTriangle({ p.v0.position.x, p.v0.position.y, p.v0.position.z },
                            { p.v1.position.x, p.v1.position.y, p.v1.position.z },
                            { p.v2.position.x, p.v2.position.y, p.v2.position.z }, ray, hitInfo)) {
                            is_hit = true;
                            if (final_t > ray.t) {
                                final_t = ray.t;
        ...
        if (state.features.extra.intersectDebug) {
            drawTriangle(debugTriangle.v0, debugTriangle.v1, debugTriangle.v2);
        }
...
BVH::Node BVH::buildLeafData(const Scene& scene, const Features& features, const AxisAlignedBox& aabb, std::span<Primitive> primitives)
{
    ...
    node.data = {
        Node::LeafBit | (uint32_t) m_primitives.size(), (uint32_t) primitives.size(), 
    };
...
void BVH::buildRecursive(const Scene& scene, const Features& features, std::span<Primitive> primitives, uint32_t nodeIndex, uint32_t level)
{
    ...
    if (primitives.size() <= BVH::LeafSize) {
        ...
        m_nodes[nodeIndex] = buildNodeData(scene, features, aabb, indexLeft, indexRight);

        buildRecursive(scene, features, primitives.subspan(0, index), indexLeft, level+1);
        buildRecursive(scene, features, primitives.subspan(index), indexRight, level+1);
    }
}
...
void BVH::debugDrawLevel(int level)
{
    ...
    helper(BVH::RootIndex, 0, level);
}

void BVH::helper(int nodeIndex, int currentLevel, int level) {
    if (currentLevel == level) {
        drawAABB(m_nodes[nodeIndex].aabb, DrawMode::Wireframe, glm::vec3(0.5f, 1.0f, 0.5f), 1.0f);
    } else {
        if (!m_nodes[nodeIndex].isLeaf()) {
            helper(m_nodes[nodeIndex].leftChild(), currentLevel + 1, level);
            helper(m_nodes[nodeIndex].rightChild(), currentLevel + 1, level);
        }
    }
}
...
{
    std::stack<BVHInterface::Node> stack;
    stack.push(m_nodes[BVH::RootIndex]);
    int currIndex = 0;
    while (!stack.empty()) {
        Node node = stack.top();
        stack.pop();
        if (node.isLeaf()) {
            if(leafIndex == currIndex) {
                drawAABB(node.aabb, DrawMode::Wireframe, glm::vec3(0.5f, 1.0f, 0.5f), 1.0f);
                break;
            }
            currIndex++;
        } else {
            stack.push(m_nodes[node.rightChild()]);
            stack.push(m_nodes[node.leftChild()]);
        }
    }
}

void BVH::sahDebug(int nodeNumber) {
    if (m_nodes[nodeNumber].isLeaf()) {
        drawAABB(m_nodes[nodeNumber].aabb, DrawMode::Wireframe, glm::vec3(1.0f, 1.0f, 1.0f), 1.0f);
    } else {
        drawAABB(m_nodes[m_nodes[nodeNumber].leftChild()].aabb, DrawMode::Wireframe, glm::vec3(1.0f, 0.0f, 0.0f), 1.0f);
        drawAABB(m_nodes[m_nodes[nodeNumber].rightChild()].aabb, DrawMode::Wireframe, glm::vec3(0.0f, 0.0f, 1.0f), 1.0f);
    }
}