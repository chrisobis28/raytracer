...
struct BVH : public BVHInterface {
    ...
    bool intersect(RenderState& state, Ray& ray, HitInfo& hitInfo) const override
    {
        ...
        if (state.features.extra.enableMotionBlur) {
            return intersectRayWithBVHTime(state, *this, ray, hitInfo);
        }
#endif
...
#if ENABLE_MOTION_BLUR
    void buildRecursiveTime(const Scene& scene, const Features& features, std::span<Primitive> primitives, uint32_t nodeIndex, uint32_t level, glm::vec2 time);
#endif
...
public: // Visual debug
    ...
    void helper(int NodeIndex, int currentLevel, int level);
#if ENABLE_MOTION_BLUR
    void debugDrawLevelTime(int level);
    void collectIndexes(int NodeIndex, int currentLevel, int level, std::stack<int>& toDraw);
#endif
    ...
    //Sah Visual Debug showing left and right children
    void sahDebug(int nodeNumber);

    //Intersection of nodes and triangles toggle for debug
    void intersectDebug();
